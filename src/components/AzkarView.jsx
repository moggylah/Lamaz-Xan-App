  ...
  return (
    <section className="azkar-screen">
      <div className="azkar-card">
        <div className="azkar-heading">
          <div>
            <h2>{t(language, 'azkar.title')}</h2>
            <p>{t(language, 'azkar.progress', { done: completedCount, total: items.length })}</p>
          </div>

          <button type="button" className="azkar-reset-all" onClick={resetCategory}>
            {t(language, 'azkar.resetAll')}
          </button>
        </div>
        ...
        <div className="azkar-category-tabs" role="tablist" aria-label={t(language, 'azkar.title')}>
          {categories.map((item) => (
            ...
            >
              ...
              <span>{t(language, item.hintKey)}</span>
            </button>
          ))}
        </div>
            ...
            return (
              <article className={`azkar-item ${complete ? 'complete' : ''}`} key={`${category}-${item.id}`}>
                <div className="azkar-item-topline">
                  <div>
                    <span className="azkar-number">
                      {t(language, 'azkar.number', { number: index + 1 })}
                    </span>
                    <h3>{getAzkarName(item, language)}</h3>
                  </div>

                  <span className="azkar-repeat-label">
                    {t(language, 'azkar.times', { count: item.repetitions })}
                  </span>
                </div>

                <p className="azkar-arabic" dir="rtl" lang="ar">
                  {item.arabic}
                </p>

                <p className="azkar-meaning">
                  {getAzkarMeaning(item, language)}
                </p>

                <div className="azkar-meta">
                  <span>{item.reference}</span>
                  <a href={AZKAR_SOURCE[category]} target="_blank" rel="noreferrer">
                    azkar.ru
                  </a>
                </div>
                ...
                <div className="azkar-counter-row">
                  ...
                  >
                    {complete ? <CheckIcon size={26} /> : count}
                  </button>

                  <div className="azkar-counter-text">
                    <strong>
                      {complete
                        ? t(language, 'azkar.done')
                        : t(language, 'azkar.remaining', { count })}
                    </strong>
                    <span>
                      {complete
                        ? t(language, 'azkar.completedHint')
                        : t(language, 'azkar.tapHint')}
                    </span>
                  </div>

                  <button
                    type="button"
                    className="azkar-reset-one"
